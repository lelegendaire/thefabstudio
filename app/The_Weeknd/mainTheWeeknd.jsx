...
const Image_3 = [
  { title: "4 win Grammy Awards",     image: "/medias/The_weeknd/grammy_Awards.webp" },
  { title: "100M+ monthly listeners", image: "/medias/The_weeknd/profile.webp" },
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      containerRef.current.querySelectorAll(".work-item").forEach((item) => {
        ...
        ScrollTrigger.create({
          trigger: item, start: "top bottom", end: "top top", scrub: 0.5,
        ...
        ScrollTrigger.create({
          trigger: item, start: "bottom bottom", end: "bottom top", scrub: 0.5,
          animation: gsap.fromTo(img,
    ...
    init();
    return () => { killed = true; stRef.current?.getAll().forEach(t => t.kill()); };
  }, []);

  // ── Three.js magazine ─────────────────────────────────────────────────────────
  useEffect(() => {
    if (typeof window === "undefined") return;
    let raf, killed = false;
    ...
    async function init() {
      ...
      const { texture: atlasTexture, imageInfos, N_ALBUMS, albums } = await buildAlbumAtlas(THREE)
      if (killed) return;
      const geo = new THREE.BoxGeometry(PAGE_W, PAGE_H, PAGE_THICKNESS, 50, 50, 1);
      ...
      instMesh.geometry.setAttribute("aTextureCoords", new THREE.InstancedBufferAttribute(texCoords, 4));
      instMesh.geometry.setAttribute("aIndex",         new THREE.InstancedBufferAttribute(idxArr, 1));
      ...
      const getWorldH   = () => camera.position.z * Math.tan(camera.fov * Math.PI / 360) * 2;
      const applyDelta  = (d) => { const v = (d * getWorldH()) / innerHeight; scrollY.target += v; mat.uniforms.uSpeedY.value += v; };
      const onWheel     = (e) => { if (!wheelReady) return; let px = e.deltaY; if (e.deltaMode===1) px*=28; if (e.deltaMode===2) px*=innerHeight; applyDelta(px); };
      const tState      = { last: 0, active: false };
      const onTStart    = (e) => { if (!wheelReady) return; tState.last = e.touches[0].clientX; tState.active = true; };
      const onTMove     = (e) => { if (!tState.active||!wheelReady) return; e.preventDefault(); applyDelta((tState.last - e.touches[0].clientX) * 2); tState.last = e.touches[0].clientX; };
      const onTEnd      = ()  => { tState.active = false; };
      const onResize    = ()  => { camera.aspect = innerWidth/innerHeight; camera.updateProjectionMatrix(); renderer.setSize(innerWidth, innerHeight); renderer.setPixelRatio(Math.min(2, devicePixelRatio)); };
...
ScrollTrigger.create({
  ...
  onUpdate: (self) => {
    ...
    scrollY.target = progress * 60; // ajuste la force ici
      ...
      // ── Render loop ───────────────────────────────────────────────────────────
      const INTRO_DUR = 5.5, SPLIT_START = 4.9, SPLIT_DUR = 1.2;
      let startTime = null;
      ...
      function animate(ts) {
        ...
        mat.uniforms.uProgress.value = easeInOut(Math.min(elapsed / INTRO_DUR, 1));
...
if (newId !== hoveredId) {
  hoveredId = newId;
  setHoveredAlbum(hoveredId >= 0 ? albums[hoveredId % N_ALBUMS] : null);
  canvas.style.cursor = hoveredId >= 0 ? "pointer" : "default";
          ...
          for (let i = 0; i < MESH_COUNT; i++) {
            ...
            if (Math.abs(next - scaleArr[i]) > 0.0001) { scaleArr[i] = next; dirty = true; }
          }
      ...
      return () => {
        ...
        renderer.dispose(); mat.dispose(); geo.dispose(); atlasTexture.dispose();
      };
    }

    let cleanupFn;
    init().then(fn => { cleanupFn = fn; });
    return () => { killed = true; cancelAnimationFrame(raf); cleanupFn?.(); };
  }, []);
useEffect(() => {
  ...
  return (
    <>
      ...
      <section className="bg-[url(/medias/The_weeknd/weekndposter.webp)] h-screen bg-cover w-screen flex items-center justify-center flex-col">
        ...
        <div className="text-white flex justify-end items-start w-full px-5 pt-5 mb-auto z-5"><OsmoMenu /></div>
        <div className="mb-auto">
          <h2 className={`${druck.className} text-9xl text-white`}>The Weeknd</h2>
          <h2 className="text-2xl text-white">One artist. One universe. One experience.</h2>
        </div>
        <div className="flex justify-between items-center text-white w-full p-5 z-2 relative">
          <p>Introducing the idol</p>
          <AnimatedLink color={"white"} onClick={() => router.push('/')} >Created by TheFabStudio</AnimatedLink>
        </div>
      </section>

      {/* Bio */}
      <section className="h-screen bg-black text-white flex items-center justify-center flex-col relative z-20">
        <Copy_bloc blockColor="#e10430"><h1 className="text-4xl font-bold">Who is he ?</h1></Copy_bloc>
        <div className="flex justify-evenly items-center">
          <div className="w-60/100">
            <Copy_bloc blockColor="#e10430">
              <p>The Weeknd, nom de scène d&apos;Abel Makkonen Tesfaye est un chanteur, acteur, auteur-compositeur-interprète et producteur canadien, né le 16 février 1990 à Toronto. Le 1er janvier 2023, son single Blinding Lights devient le plus écouté de tous les temps sur Spotify, dépassant Shape of You d&apos;Ed Sheeran.</p>
            </Copy_bloc>
          </div>
          <div><Image src="/medias/The_weeknd/weeknd_pose.webp" width={300} height={700} alt="The weeknd Picture" className="rounded-2xl" /></div>
        </div>
      </section>

      {/* Scroll images */}
      <section ref={containerRef} className="h-full bg-black text-white flex items-center justify-center flex-col">
        {Image_3.map((work, index) => (
          <div key={index} className="work-item h-[150vh] relative overflow-hidden w-screen">
            <div className="work-item-img absolute w-full h-full [clip-path:polygon(25%_25%,75%_40%,100%_100%,0%_100%)] will-change-[clip-path]">
              <img src={work.image} alt={work.title} className={`w-full h-full object-cover ${index === 1 ? "object-center" : "object-top"}`} />
            </div>
            <div className="work-item-name absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full z-1">
              <h1 className="text-6xl md:text-8xl font-bold text-white uppercase tracking-tight">{work.title}</h1>
            </div>
          </div>
        ))}
      ...
      {/* WebGL Magazine */}
      <section ref={sectionRef} className="h-screen w-screen relative bg-black overflow-hidden">
        <canvas
          ref={canvasRef}
        ...
        <div style={{
          position: "absolute", top: "4vmax", left: "50%",
        ...
        {/* Scroll hint */}
        <p style={{
          position: "absolute", bottom: "4vmax", left: "50%",
          transform: "translateX(-50%)",
      ...
      <section className="h-screen relative w-full bg-black text-white">
            <div className="video-container relative right-0 bottom-0 w-screen h-screen min-w-full min-h-full overflow-hidden">
              <div id="youtube-player" className="w-full h-full pointer-events-none" />
            </div>
            <div ref={cursorRef} className="cursor absolute top-5 left-5 uppercase  transition-transform duration-100 ease-[cubic_bezier(0.075_0.82_0.165_1)] z-2 pointer-events-none"><p>{isPlaying ? "Pause" : "Play"}</p></div>
            <div ref={timelineRef} onClick={handleTimelineClick} className="video-timeline absolute bottom-0 mb-5 w-screen h-40 flex flex-col p-4 gap-2 cursor-pointer ">
              <div ref={markerRef} className="video-marker absolute bottom-3 left-0 w-0.5 h-37.5 bg-red-500 transition-[left] duration-500 z-2 ease-linear before:absolute before:content-[''] before:top-0 before:left-1/2 before:transform before:-translate-x-1/2 before:-translate-y-1/2 before:w-2.5 before:h-2.5 before:rounded-[100%] before:bg-red-500"></div>
              <div className="video-timestamps relative w-full flex justify-between ">
                <p>00:00</p>
                <p>00:05</p>
                <p>00:10</p>
                <p>00:15</p>
              ...
              <div className="video-frames h-full w-full flex justify-between gap-4 border-t border-dashed border-t-white border-b  border-b-white px-0 py-3">
                <div className='frame relative after:content-[""] after:absolute after:top-0 after:left-0 after:w-full after:h-full after:bg-[#00000080] hover:after:bg-black duration-300 transition'>
                  <img src={"/medias/Blade_runner.webp"} className="h-full w-full object-cover"></img>
                </div>
                <div className='frame relative after:content-[""] after:absolute after:top-0 after:left-0 after:w-full after:h-full after:bg-[#00000080] hover:after:bg-black duration-300 transition'>
                  <img src={"/medias/Blade_runner.webp"} className="h-full w-full object-cover"></img>
                </div>
